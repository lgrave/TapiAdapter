...
//
//	$Log: XmlRpcClient.cpp,v $
//	Revision 1.2.2.1  2010-07-22 10:16:38  lgrave
//	RT8: TapiAdapter: threads for reception/execution of paralel commands
//
//	Revision 1.2  2010-07-21 14:21:41  lgrave
//	random in the url of events to avoid cache issues
//
...
XmlRpcClient::execute(const char* method, XmlRpcValue const& params, XmlRpcValue& result)
{
  ...
  if ( ! setupConnection())
    throw "setupConnection"; //return false;

  if ( ! generateRequest(method, params))
    throw "generateRequest"; //return false;
...
{
  char random[1024];
  sprintf(random, "random=%d", rand());
  std::string header = 
    "POST " + _uri + "?" + random + " HTTP/1.1\r\n"
    "User-Agent: ";
...
//XmlRpcUtil::error((char*)_header.c_str());
